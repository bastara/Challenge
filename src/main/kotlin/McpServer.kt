...
fun main() {
    ...
    if (apiKey.isNullOrBlank()) {
        ...
        println("Установите её: set OPENWEATHER_API_KEY=ваш_ключ")
        return
    }
    ...
    server.createContext("/mcp") { exchange ->
        if (exchange.requestMethod == "POST") {
            ...
            val response = handleMcpRequest(request, apiKey)
...
fun handleMcpRequest(request: JSONObject, apiKey: String): JSONObject {
                    ...
                    put("parameters", JSONObject().apply {
                        ...
                        put("properties", JSONObject().apply {
                            put("city", JSONObject().apply {
                                put("type", "string")
                                put("description", "Название города на английском, например 'Moscow' или 'London'")
                            })
                        })
        ...
        "tools/call" -> {
            ...
            val result = when (toolName) {
                "get_weather" -> {
                    ...
                    if (city.isBlank()) {
                        JSONObject().apply { put("error", "Не указан город (параметр 'city')") }
                    } else {
                        getWeather(city, apiKey)
                    }
                }
                else -> JSONObject().apply { put("error", "Неизвестный инструмент: $toolName") }
            }
...
fun getWeather(city: String, apiKey: String): JSONObject {
    ...
    return try {
        ...
        if (responseCode == 200) {
            ...
            JSONObject().apply {
                put("error", "HTTP $responseCode: ${connection.responseMessage}")
            }
        }
    } catch (e: Exception) {
