...
// ----- Рой агентов -----
class SwarmOrchestrator(private val apiKey: String) {
    private val agents = mapOf(
        "Аналитик" to TaskAgent(apiKey, systemPrompt = "Ты — системный аналитик. Твоя задача — собрать и уточнить требования к проекту. Не предлагай технических решений, только требования и ограничения."),
        "Архитектор" to TaskAgent(apiKey, systemPrompt = "Ты — архитектор программного обеспечения. На основе требований предложи структуру API, базы данных, модулей. Используй Kotlin и Ktor."),
        "Кодер" to TaskAgent(apiKey, systemPrompt = "Ты — опытный Kotlin-разработчик. Напиши конкретный код на Kotlin с использованием Ktor для реализации предложенной архитектуры. Включай только необходимые эндпоинты."),
        "Критик" to TaskAgent(apiKey, systemPrompt = "Ты — строгий ревьюер. Проверь предложенное решение на соответствие инвариантам, лучшим практикам, безопасности. Предложи улучшения или укажи на ошибки.")
    )
    ...
    fun execute(task: String): String {
        ...
        for ((role, agent) in agents) {
            println("--- $role работает... ---")
            val answer = agent.sendMessage(currentInput)
            results.add(role to answer)
            println("[$role]: $answer")
            currentInput = answer
        }

        return buildString {
            appendLine("=== Результат роя ===")
            results.forEach { (role, answer) ->
                appendLine("[$role]")
                appendLine(answer)
                appendLine()
            }
...
fun main() {
    ...
    println("Агент с конечным автоматом, инвариантами и роем")
    println("Команды:")
    ...
    println("  swarm <задача>            – запустить рой из 4 специалистов")
    ...
    while (true) {
        ...
        when {
            ...
            input.startsWith("swarm ", true) -> {
                val orchestrator = SwarmOrchestrator(apiKey)
                val task = input.removePrefix("swarm ").trim()
                println("Запуск роя для задачи: $task")
                val result = orchestrator.execute(task)
                println(result)
            }
